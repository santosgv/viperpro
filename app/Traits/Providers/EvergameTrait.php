...
trait EvergameTrait
{
    ...
    private static function PrepareTransactionsEvergame($wallet, $userCode, $txnId, $betMoney, $winMoney, $gameCode, $providerCode, $changeBonus, $txnType)
    {
        ...
        if($winMoney > $betMoney) {
            $hash = Helper::GenerateHash('hash:'.$userCode.'-'.$gameCode); # 1.5.0
            $checkBetTransaction = Order::where('transaction_id', $txnId)
                ->where('type', 'bet')
                ->where('status', 0)
                ->where('hash', $hash)
                ->first();

            if(!empty($checkBetTransaction)) {
                $transaction = self::CreateTransactionsEvergame($userCode, time(), $txnId, 'win', $changeBonus, $winMoney, $gameCode, $gameCode);
            ...
            $hash = Helper::GenerateHash('hash:'.$userCode.'-'.$gameCode); # 1.5.0
            $checkTransaction = Order::where('transaction_id', $txnId)
                ->where('type', 'bet')
                ->where('status', 0)
                ->where('hash', $hash)
                ->first();
            ...
            if(empty($checkTransaction)) {
                $checkTransaction = self::CreateTransactionsEvergame($userCode, time(), $txnId, 'bet', $changeBonus, $betMoney, $gameCode, $gameCode);
            }
    ...
    private static function CreateTransactionsEvergame($playerId, $betReferenceNum, $transactionID, $type, $changeBonus, $amount, $game, $pn)
    {
        ...
        $hash = Helper::GenerateHash('hash:'.$playerId.'-'.$game); # 1.5.0
        $order = Order::create([
            'user_id'       => $playerId,
            ...
            if ($response->successful()) {
                ...
                if (isset($data['vendors'])) { // Check if 'vendors' key exists in the response
                    foreach ($data['vendors'] as $vendor) { // Iterate through vendors
                        ...
                        $checkProvider = Provider::where('code', strtolower($vendor['vendorCode']))->first();
